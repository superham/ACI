...
class TestComputeACI:
    ...
    def test_multiple_groups(self):
        df = pd.DataFrame([
            {"group": "a", "sample_offer_rate": 1.0, "key_delivery_rate": 1.0,
             "publish_rate": 1.0, "leak_threat_rate": 1.0,
             "violation_claim_rate": 0.0, "reextortion_behavior_rate": 0.0},
            {"group": "b", "sample_offer_rate": 0.0, "key_delivery_rate": 0.0,
             "publish_rate": 0.0, "leak_threat_rate": 0.0,
             "violation_claim_rate": 1.0, "reextortion_behavior_rate": 1.0},
        ])
        result = compute_aci(df)
        assert len(result) == 2
...
class TestConfidence:
    ...
    def test_low_data(self):
        df = pd.DataFrame([{
            "n_chats": 1, "total_claims": 2,
            "R": 0.5, "T": np.nan, "I": np.nan,
        }])
        conf = _compute_confidence(df)
    ...
    def test_no_data(self):
        df = pd.DataFrame([{
            "n_chats": 0, "total_claims": 0,
            "R": np.nan, "T": np.nan, "I": np.nan,
        }])
        conf = _compute_confidence(df)
